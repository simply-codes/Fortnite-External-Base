...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    const auto colA = ImColor(50, 50, 50, 255);
    const auto colB = ImColor(50, 50, 50, 255);
    // window->DrawList->AddRectFilledMultiColor(bb.Min, bb.Max, colA, colA, colB, colB);
    const auto colBorder = ImGui::GetColorU32(ImGui::GetStyleColorVec4(ImGuiCol_Border));
    ImGui::GetWindowDrawList()->AddRect(bb.Min, bb.Max, colBorder, 0.f, 15, 1.f);
...
bool ImGui::g_ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const auto colB = ImGui::GetColorU32(ImGui::GetStyleColorVec4(ImGuiCol_FrameBg));
    // window->DrawList->AddRectFilledMultiColor(bb.Min, bb.Max, colA, colA, colB, colB);
    const auto colBorder = ImGui::GetColorU32(ImGui::GetStyleColorVec4(ImGuiCol_Border));
    ImGui::GetOverlayDrawList()->AddRect(bb.Min, bb.Max, colBorder, 0.f, 15, 1.f);
...
bool ImGui::g_Button(const char* label, const ImVec2& size_arg)
{
    return g_ButtonEx(label, size_arg, ImGuiButtonFlags_None);
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const float square_sz = labelSize.y;//GetFrameHeight() * 0.6f;
    const float offset = (GetFrameHeight() - square_sz) * 0.5f;
    const ImVec2 pos = window->DC.CursorPos + ImVec2(offset, offset);
    const ImRect total_bb(pos, pos + ImVec2(square_sz + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), label_size.y + style.FramePadding.y * 2.0f));
    ItemSize(total_bb, style.FramePadding.y);
    if (!ItemAdd(total_bb, id))
        return false;

    const ImVec2 stPos = pos - ImVec2(offset, offset);
    //window->DrawList->AddRectFilled(stPos, stPos + ImVec2(GetFrameHeight(), GetFrameHeight()), ImColor(0, 255, 0));
    ...
    RenderFrame(check_bb.Min, check_bb.Max, GetColorU32((held && hovered) ? ImGuiCol_FrameBg : hovered ? ImGuiCol_FrameBg : ImGuiCol_FrameBg), true, style.FrameRounding);
    ...
    const auto colBorder = ImGui::GetColorU32(ImGui::GetStyleColorVec4(ImGuiCol_Border));
    window->DrawList->AddRect(check_bb.Min, check_bb.Max, colBorder, 0.f, 15, 1.f);

    if (g.LogEnabled)
        LogRenderedText(&total_bb.Min, mixed_value ? "[~]" : *v ? "[x]" : "[ ]");
    if (label_size.x > 0.0f) {
        const auto center = (square_sz - labelSize.y) * 0.5f;
        RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + center - 0.5f), label);
    }
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImU32 frame_col = GetColorU32(hovered ? ImGuiCol_Border : ImGuiCol_Border);
    const float value_x2 = ImMax(frame_bb.Min.x, frame_bb.Max.x - arrow_size);
    RenderNavHighlight(frame_bb, id);
    if (!(flags & ImGuiComboFlags_NoPreview))
        window->DrawList->AddRect(frame_bb.Min, ImVec2(value_x2 + 1, frame_bb.Max.y), frame_col, style.FrameRounding, (flags & ImGuiComboFlags_NoArrowButton) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Left);
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ImU32 bg_col = GetColorU32((popup_open || hovered) ? ImGuiCol_Border : ImGuiCol_Border);
        ImU32 text_col = GetColorU32(ImGuiCol_Text);
        window->DrawList->AddRect(ImVec2(value_x2, frame_bb.Min.y), frame_bb.Max, bg_col, style.FrameRounding, (w <= arrow_size) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Right);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddRectFilled(total_bb.Min, { grab_bb.Max.x, total_bb.Max.y }, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
    //window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);

    const auto colBorder = ImGui::GetColorU32(ImGui::GetStyleColorVec4(ImGuiCol_Border));
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, colBorder, 0.f, 0, 1.f);
    ...
    const auto center = ((frame_bb.Max.y - frame_bb.Min.y) - label_size.y) * 0.5f;
    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Min.x , frame_bb.Min.y - 15), label);
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    float wheel_r_outer = sv_picker_size * 0.10f;
    ...
    const int style_alpha8 = IM_F32_TO_INT8_SAT(1.0f);
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        draw_list->AddCircleFilled(hue_cursor_pos, hue_cursor_rad, hue_color32, 1);
        draw_list->AddCircle(hue_cursor_pos, hue_cursor_rad + 1, col_midgrey, 1);
        draw_list->AddCircle(hue_cursor_pos, hue_cursor_rad, col_white, 1);
    ...
    else if (flags & ImGuiColorEditFlags_PickerHueBar)
    {
        ...
        sv_cursor_pos.x = ImClamp(IM_ROUND(picker_pos.x + ImSaturate(S) * sv_picker_size), picker_pos.x + 2, picker_pos.x + sv_picker_size - 10); // Sneakily prevent the circle to stick out too much
        sv_cursor_pos.y = ImClamp(IM_ROUND(picker_pos.y + ImSaturate(1 - V) * sv_picker_size), picker_pos.y + 2, picker_pos.y + sv_picker_size - 10);
    ...
    draw_list->AddCircleFilled(sv_cursor_pos, sv_cursor_rad, user_col32_striped_of_alpha, 15);
    draw_list->AddCircle(sv_cursor_pos, sv_cursor_rad + 1, col_midgrey, 15);
    draw_list->AddCircle(sv_cursor_pos, sv_cursor_rad, col_white, 15);
